...
HDPrivateKey.prototype.derive32 = function derive32(index) {
  console.log("====>D32", index);
  ...
  var prKey = bs58.encode(key);
  var pub;

  // var secpAsync = co.promisify(secp256k1.default.publicForPrivate);
  // var pub = secpAsync(prKey).then(function(publicKey,error) {
  //   if (error) {
  //     console.error("PBKEY ERROR:", error);
  //     return error;
      
  //   } else {
  //     console.log('PUB:', bs58.decode({publicKey}));
  //     return {publicKey};
      
  //   }
  // });

  // console.log('REAL public key for private result:', child.publicKey);
...
HDPrivateKey.prototype.deriveAccount32 = function deriveAccount32(accountIndex, cache) {
  assert(util.isNumber(accountIndex), 'Account index must be a number.');
  assert(this.isMaster(), 'Cannot derive account index.');
...
HDPrivateKey.prototype.derivePath32 = function derivePath(path, cache) {
