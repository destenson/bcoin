...
/**
 * Exports the tree as a hex Buffer.
 * @returns {Buffer} tree
 */
...
RBT.prototype.fromOptions = function fromOptions(opt) {  
  if (opt.rbtRaw) {
    assert(opt.rbtRaw.length > 0)
    this.fromRaw(opt.rbtRaw)
  }
  return this
}

/**
 * Creates a new RBT from options
 * @param {object} opt
 * @returns {RBT} new RBT
 */
...
RBT.fromOptions = function fromOptions(opt) {  
  ...
  return new RBT(opt.compare).fromOptions(opt)
}
