...
function MemDB(location, options) {
...
MemDB.fromOptions = function fromOptions(options) {
  return new MemDB(options.location, options)
}

MemDB.prototype.fromOptions = function fromOptions(options) {
  let rbtRaw = null;

  if (options.rbtRaw) {
    assert(Buffer.isBuffer(options.rbtRaw), 'rbtRaw must be a Buffer.');
    rbtRaw = options.rbtRaw;
  }

  this.tree = RBT.fromOptions({ compare: cmp, rbtRaw });
  
  return this;
};

MemDB.prototype.toRaw = function toRaw(options) {
  const locationLen = this.location.length;
  const treeData = this.tree.toRaw();
  const treeDataLength = treeData.length;
  
  const size = treeDataLength + locationLen + 8 * 2;
  const bw = new StaticWriter(size);
  
  bw.writeU64(locationLen);
  bw.writeString(this.location);
  bw.writeU64(treeDataLength);
  bw.writeBytes(this.treeData);
  
  return bw.render();
};

MemDB.fromRaw = function fromRaw(data) {
  const br = new BufferReader(data);
  
  const locationLen = br.readU64();
  const location = br.readString(locationLen);
  const treeDataLength = br.readU64();
  const treeData = br.readBytes(treeDataLength);

  return new MemDB(location, {rbtRaw: treeData});
};
