...
Account.prototype.createKey = co(function* createKey(branch) {
  ...
  switch (branch) {
    case 0:
      key = yield this.deriveReceive(this.receiveDepth);
      lookahead = yield this.deriveReceive(this.receiveDepth + this.lookahead);
      yield this.saveKey(lookahead);
      this.receiveDepth++;
      this.receive = key;
      break;
    case 1:
      key = yield this.deriveChange(this.changeDepth);
      lookahead = yield this.deriveReceive(this.changeDepth + this.lookahead);
      yield this.saveKey(lookahead);
      this.changeDepth++;
      this.change = key;
      break;
    case 2:
      key = yield this.deriveNested(this.nestedDepth);
      lookahead = yield this.deriveNested(this.nestedDepth + this.lookahead);
      yield this.saveKey(lookahead);
      this.nestedDepth++;
      this.nested = key;
      break;
...
Account.prototype.deriveReceive = co(function* deriveReceive(index, master) {
  return yield this.deriveKey(0, index, this.wallet.master);
});
...
Account.prototype.deriveChange = function deriveChange(index, master) {
  return this.deriveKey(1, index, this.wallet.master);
};
...
Account.prototype.deriveNested = function deriveNested(index, master) {
  ...
  return this.deriveKey(2, index, this.wallet.master);
};
...
Account.prototype.derivePath = co(function* derivePath(path, master) {
  var data = path.data;
  var ring;
...
Account.prototype.deriveKey = co(function* deriveKey(branch, index, master) {
  var keys = [];
  ...
  if (master && master.key && !this.watchOnly) {
    // key = master.key.deriveAccount32(this.accountIndex);
    
    // key = master.key.derive32(0).derive32(branch).derive32(index);
    key = yield master.key.derive32(0);
    key = yield key.derive32(branch);
    key = yield key.derive32(index);
  } else {
    
    //44=> key = this.accountKey.derive(branch).derive(index); 
    /*32=>*/ //key = this.accountKey.derive32(0).derive32(branch).derive32(index); 
    key = yield this.accountKey.derive32(0);
    key = yield key.derive32(branch);
    key = yield key.derive32(index);
  }
  ...
  switch (this.type) {
    ...
    case Account.types.MULTISIG:
      ...
      for (i = 0; i < this.keys.length; i++) {
        shared = this.keys[i];
        shared = yield shared.derive32(0);
        shared = yield shared.derive32(branch);
        shared = yield shared.derive32(index);
...
Account.prototype.saveKey = co(function* saveKey(ring) {
  return yield this.db.saveKey(this.wallet, ring);
});
...
Account.prototype.initDepth = co(function* initDepth() {
  ...
  // Receive Address
  this.receive = yield this.deriveReceive(0);
  this.receiveDepth = 1;
  ...
  for (i = 0; i < this.lookahead; i++) {
    key = yield this.deriveReceive(i + 1);
    yield this.saveKey(key);
  }

  // Change Address
  this.change = yield this.deriveChange(0);
  this.changeDepth = 1;
  ...
  // Lookahead
  for (i = 0; i < this.lookahead; i++) {
    key = yield this.deriveChange(i + 1);
    yield this.saveKey(key);
  }

  // Nested Address
  if (this.witness) {
    this.nested = yield this.deriveNested(0);
    this.nestedDepth = 1;
    ...
    // Lookahead
    for (i = 0; i < this.lookahead; i++) {
      key = yield this.deriveNested(i + 1);
      yield this.saveKey(key);
    }
  }
...
Account.prototype.syncDepth = co(function* syncDepth(receive, change, nested) {
  ...
  if (receive > this.receiveDepth) {
    ...
    for (i = depth; i < receive + this.lookahead; i++) {
      key = yield this.deriveReceive(i);
      yield this.saveKey(key);
    }

    this.receive = yield this.deriveReceive(receive - 1);
  ...
  if (change > this.changeDepth) {
    ...
    for (i = depth; i < change + this.lookahead; i++) {
      key = yield this.deriveChange(i);
      yield this.saveKey(key);
    }

    this.change = yield this.deriveChange(change - 1);
  ...
  if (this.witness && nested > this.nestedDepth) {
    ...
    for (i = depth; i < nested + this.lookahead; i++) {
      key = yield this.deriveNested(i);
      yield this.saveKey(key);
    }

    this.nested = yield this.deriveNested(nested - 1);
...
Account.prototype.setLookahead = co(function* setLookahead(lookahead) {
  ...
  if (lookahead < this.lookahead) {
    ...
    this.receive = yield this.deriveReceive(this.receiveDepth - 1);

    this.changeDepth += diff;
    this.change = yield this.deriveChange(this.changeDepth - 1);

    if (this.witness) {
      this.nestedDepth += diff;
      this.nested = yield this.deriveNested(this.nestedDepth - 1);
    }
