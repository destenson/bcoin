...
MTX.prototype.template = co(function* template(ring) {
  var total = 0;
  var i, input, coin;
 
  if (Array.isArray(ring)) {
    for (i = 0; i < ring.length; i++)
      total += yield this.template(ring[i]);
    return total;
  }

  for (i = 0; i < this.inputs.length; i++) {
    input = this.inputs[i];
    coin = yield this.view.getOutput(input);
