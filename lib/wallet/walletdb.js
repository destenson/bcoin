...
WalletDB.prototype._create = co(function* create(options) {
  ...
  wallet = yield Wallet.fromOptions(this, options);
...
WalletDB.prototype.saveKey = co(function* saveKey(wallet, ring) {
  yield ring;
...
/**
 * Add a transaction from raw tx data to the database, map addresses
 * to wallet IDs, potentially store orphans, resolve
 * orphans, or confirm a transaction.
 * @param {TX} tx
 * @returns {Promise}
...
WalletDB.prototype.addTXFromRaw = co(function* addTX(txData) {
  var unlock = yield this.txLock.lock();
  ...
  try {
    var txd = hexToBytes(txData);
    // console.log("TXData: ",txd);
    var tx = TX.fromRaw(txd);
    return yield this._insert(tx);
  } finally {
    unlock();
  }
