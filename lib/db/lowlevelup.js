...
LowlevelUp.prototype.init = function init() {
  ...
  let db;
  if (Backend.name === 'MemDB' && this.options.memDbOptions) {
    db = new Backend(this.location, this.options.memDbOptions);
...
LLUOptions.prototype.fromOptions = function fromOptions(options) {
  ...
  if (options.memDbOptions != null) {
    assert(typeof options.memDbOptions === 'object',
      '`memDbOptions` must be an object.');
    this.memDbOptions = options.memDbOptions;
  }
