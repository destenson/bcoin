...
HDPublicKey.prototype.verifyNetwork = function verifyNetwork(network) {
  ...
  return this.network.keyPrefix.xpubkey === network.keyPrefix.xpubkey && this.network.keyPrefix.coinType === network.keyPrefix.coinType;
};
...
HDPublicKey.prototype.derive = function derive(index, hardened, cache) {
  ...
  if (index >= common.HARDENED || hardened) throw new Error('Index out of range.');
...
HDPublicKey.prototype.derive32 = function derive32(index) {
  var bw, id, data, hash, left, right, key, child;
  ...
  if (isHardened) { // Hardened child
    assert(this.publicKey, 'Could not derive hardened child key')
...
HDPublicKey.prototype.deriveAccount32 = function deriveAccount32(accountIndex, cache) {
  return this;
};
...
HDPublicKey.prototype.derivePath32 = function derivePath(path, cache) {
  ...
  entries.forEach(function (c, i) {
    ...
    hdkey = hdkey.derive32(childIndex)
  })
...
HDPublicKey.prototype.equal = function equal(obj) {
  ...
  return this.network === obj.network && this.depth === obj.depth && util.equal(this.parentFingerPrint, obj.parentFingerPrint) && this.childIndex === obj.childIndex && util.equal(this.chainCode, obj.chainCode) && util.equal(this.publicKey, obj.publicKey);
};
...
HDPublicKey.prototype.compare = function compare(key) {
  ...
  if (!HDPublicKey.isHDPublicKey(key)) return 1;
...
HDPublicKey.prototype.fromBase58 = function fromBase58(xkey, network) {
  ...
  this._xpubkey = xkey;
  if (network && !this.verifyNetwork(network)) throw new Error('Network mismatch for HD public key.');
  return this;
};
...
HDPublicKey.isHDPublicKey = function isHDPublicKey(obj) {
  return obj && typeof obj.derive === 'function' && typeof obj.toExtended !== 'function' && obj.chainCode !== undefined;
};
