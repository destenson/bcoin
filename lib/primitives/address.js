...
Address.prototype.toBase58 = function toBase58(network) {
  ...
  /*BIP32:*/ return this.toBase58Check(network);
};



/**
 * Compile the address object to a base58check address.
 * @param {{NetworkType|Network)?} network
 * @returns {Base58Address}
 * @throws Error on bad hash/prefix.
 */

Address.prototype.toBase58Check = function toBase58Check(network) {

  var bw = new StaticWriter(21);

  bw.writeU8(0x00, 0);
  bw.writeBytes(this.hash);
  
  var res = bs58.encode(bw.render());
