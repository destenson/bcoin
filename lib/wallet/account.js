...
Account.prototype.fromOptions = function fromOptions(options) {
  ...
  if (options.pureBip32Mode) {
    assert(options.masterPath);
    assert(HD.common.parsePath(options.masterPath, false));
    this.pureBip32Mode = true;
...
Account.prototype.deriveKey = function deriveKey(branch, index, master) {
  ...
  if (master && master.key && !this.watchOnly) {
    if (this.pureBip32Mode) {
      key = master.key.derivePath(this.masterPath);
