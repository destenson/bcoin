...
var BBuffer = require('buffer/').Buffer
var bs58 = require('bs58check')

var convertToAsyncWithPromise = function(fn) {
    return function() {
...
HDPrivateKey.prototype.derive32 = co(function* derive32(index) {
  console.log("PR====>D32", index);
  var bw, id, data, hash, left, right, key, child;
  
  var isHardened = index >= HARDENED_OFFSET
  var indexBuffer = new BBuffer(4)
  ...
  var I = crypto.hmac('sha512', data, this.chainCode)
  var left = I.slice(0, 32)
  var right = I.slice(32)
  ...
  var prKey = key.toString("hex")

  if (typeof secp256k1 != "undefined"){
    var secpAsync = co.promisify(secp256k1.default.publicForPrivate);
  
    var pub = yield secpAsync(prKey).then(function(publicKey,error) {
      if (error) {
        console.error("PBKEY ERROR:", error);
        return error;
      } else {
        // console.log('PUB2:', publicKey);
        var result = hexToBytes(publicKey);
        // console.log('PUB2:', result);
        return result;
      }
    });
  } else {
    var pub = yield ec.publicKeyCreate(key, true);
  }

  // console.log('REAL public key for private result:', pub);
...
HDPrivateKey.prototype.derive = function derive(index, hardened, cache) {
  ...
  if (!cache) cache = common.cache;

  if (typeof index === 'string') return this.derivePath(index, cache);

  hardened = index >= common.HARDENED ? true : hardened;

  if (index < common.HARDENED && hardened) index += common.HARDENED;
...
HDPrivateKey.prototype.getID = function getID(index) {
  ...
  return network.keyPrefix.xprivkey58 + this.publicKey.toString('hex') + index;
};
...
HDPrivateKey.prototype.deriveAccount44 = function deriveAccount44(accountIndex, cache) {
  ...
  return this.derive(44, true, cache).derive(network.keyPrefix.coinType, true, cache).derive(accountIndex, true, cache);
};


/**
 * Derive a BIP32 account key.
 * @param {Number} accountIndex
 * @returns {HDPrivateKey}
 * @throws Error if key is not a master key.
 */

HDPrivateKey.prototype.deriveAccount32 = co(function* deriveAccount32(accountIndex, cache) {
  assert(util.isNumber(accountIndex), 'Account index must be a number.');
  assert(this.isMaster(), 'Cannot derive account index.');
  return this.derivePath32("m/0/0/"+accountIndex.toString());
});
...
HDPrivateKey.isBase58 = function isBase58(data) {
  ...
  for (i = 0; i < networks.types.length; i++) {
    ...
    prefix = network.keyPrefix.xprivkey58;
...
HDPrivateKey.isRaw = function isRaw(data) {
  ...
  for (i = 0; i < networks.types.length; i++) {
    ...
    prefix = network.keyPrefix.xprivkey;
    if (version === prefix)
      return type;
  }
...
HDPrivateKey.prototype.derivePath32 = co(function* derivePath(path, cache) {

  console.log("====>DP32", path);
  ...
  for (var k=0;k<=dp.length-1;k++){
    hdkey = yield hdkey.derive32(dp[k]);
  }
  ...
  // var dp$1 = dp;
  // if (dp.length==3){
  //   return hdkey.derive32(dp[0]).then(function(res){
  //     return res.derive32(dp$1[1]);
  //   }).then(function(res){
  //     return res.derive32(dp$1[2]);
  //   })  
  // }
