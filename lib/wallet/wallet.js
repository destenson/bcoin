...
var backend = require('../crypto/backend-browser');
...
Wallet.prototype.fromOptions = co(function* fromOptions(options) {
  ...
  if (!token)
    token = yield this.getToken(this.tokenDepth);
...
Wallet.fromOptions = co(function* fromOptions(db, options) {
  return yield new Wallet(db).fromOptions(options);
});
...
Wallet.prototype.getID = co(function* getID() {
  var bw, key, hash;

  
  key = yield this.master.key.derive32(0);
  key = yield key.derive32(0);
  key = yield key.derive32(0);

  var identifier = backend.hash160(key.publicKey);

  var payload = new BBuffer(21);
  
  payload.writeUInt8(0x00, 0);

  identifier.copy(payload, 1);
 
  bw = new StaticWriter(21);
...
Wallet.prototype.getToken = co(function* getToken(nonce) {
  
  var bw, key;

  assert(this.master.key, 'Cannot derive token.');

  return key = this.master.key.derive32(0)
    .then(function(key){
      return key.derive32(0);
    })
    .then(function(key){
      return key.derive32(0);
    }).then(function(key){
...
Wallet.prototype._createAccount = co(function* createAccount(options, passphrase) {
  ...
  // console.log("ACCOUNT KEY", this.master)
  ...
  if (this.watchOnly && options.accountKey) {
    ...
    key = yield this.master.key.deriveAccount32(this.accountDepth);
    key = key.toPublic();
  }
...
Wallet.prototype.deriveInputs = co(function* deriveInputs(mtx) {
  ...
  for (i = 0; i < paths.length; i++) {
    ...
    ring = yield account.derivePath(path, this.master);
...
Wallet.prototype._add = co(function* add(tx, block) {
  ...
  // console.log("adding-", tx.hash("hex"));
