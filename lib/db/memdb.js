...
MemDB.prototype.fromOptions = function fromOptions(options) {
  ...
  let backup = null;
  if (options.compare) {
    assert(typeof options.compare === 'function', 'Compare must be a Function.')
    compare = options.compare;
  }
  
  if (options.unique !== null && options.unique !== undefined) {
    assert(typeof options.unique === 'boolean', 'Unique must be a Boolean.');
    unique = options.unique;
  }
  if (options.backup) {
    assert(Buffer.isBuffer(options.backup), 'Backup must be a Buffer.');
    backup = options.backup;
  }
  
  this.tree = RBT.fromOptions({ compare, unique, backup });
